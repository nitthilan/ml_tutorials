...
def fire_module(x, fire_id, squeeze=16, expand=64, use_bias=True):
    ...
    x = Convolution2D(squeeze, (1, 1), padding='valid', use_bias=use_bias, 
        kernel_constraint=unit_norm(axis=[0, 1, 2]), 
        name=s_id + sq1x1)(x)
    x = Activation('relu', name=s_id + relu + sq1x1)(x)

    left = Convolution2D(expand, (1, 1), padding='valid', use_bias=use_bias, 
        kernel_constraint=unit_norm(axis=[0, 1, 2]), 
        name=s_id + exp1x1)(x)
    left = Activation('relu', name=s_id + relu + exp1x1)(left)

    right = Convolution2D(expand, (3, 3), padding='same', use_bias=use_bias, 
        kernel_constraint=unit_norm(axis=[0, 1, 2]), 
        name=s_id + exp3x3)(x)
...
def SqueezeNet(include_top=True, weights='imagenet',
    ...
    F1=int(num_filter*16); FS2= int(num_filter*4); 
    FE2=int(num_filter*16); FS3=int(num_filter*8); 
    FE3= int(num_filter*32); FS4=int(num_filter*12); 
    FE4= int(num_filter*48); FS5=int(num_filter*16); 
    FE5=int(num_filter*64);
