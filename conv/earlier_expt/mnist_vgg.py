...
# ../../data/conv/vgg_weights/

class cifar10vgg:
    def __init__(self,train=False):
        self.num_classes = 10
        self.weight_decay = 0.0005
        self.x_shape = [32, 32, 1] # [32,32,3]
    ...
    def train(self,model):
        ...
        print(x_train.shape, x_test.shape, 
            x_train_zeros.shape, x_test_zeros.shape)

        x_train = x_train.astype('float32')
        x_test = x_test.astype('float32')
        x_train_zeros[:,2:30, 2:30, 0] = x_train
        x_test_zeros[:,2:30, 2:30, 0] = x_test
        x_train, x_test = self.normalize(x_train_zeros, x_test_zeros)
...
if __name__ == '__main__':
    ...
    model = cifar10vgg()
    x_train, x_test = model.normalize(x_train_zeros, x_test_zeros)
    ...
    print("Predicted Shape ", predicted_x.shape)
    residuals = np.argmax(predicted_x,1)!=np.argmax(y_test,1)

    print(predicted_x.shape, x_test.shape, residuals.shape)
    print(predicted_x[:10], y_test[:10])
    print(np.argmax(predicted_x,1)[:10], np.argmax(y_test,1)[:10],residuals[:10])

    loss = 1 - (sum(residuals)*1.0/len(residuals))
