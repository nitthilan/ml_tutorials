...
#   - 
# WGAN: lipschitz function examples
#   - kl divergence example, Total Variation (TV) distance
# WGAN:
...
def get_sample(num_samples, sample_dim, num_ones):
    ...
    # print(zeros_val[:10])
    # print(index[:50])
    mean_val = (sample_dim - 2*num_ones)*1.0/sample_dim
...
def generate_images(generator_model, output_dir, epoch, noise_dim):
    """Feeds random seeds into the generator and tiles and saves the output to a PNG file."""
    test_image_stack = generator_model.predict(np.random.rand(10, noise_dim))
    ...
    print(test_image_stack)
    test_image_stack_1 = np.copy(test_image_stack)
    test_image_stack_1[test_image_stack <   -0.8] = -1
    test_image_stack_1[test_image_stack >=  -0.8] = 1
    print(test_image_stack_1)
    test_image_stack_2 = np.copy(test_image_stack)
    test_image_stack_2[test_image_stack <= 0] = 0
    test_image_stack_2[test_image_stack >  0] = 1
    print(test_image_stack_2)
    print(np.sum(test_image_stack_1, axis=1))
    print(np.sum(test_image_stack_2, axis=1))
...
X_train = get_sample(num_samples, sample_dim, num_ones)

# Now we initialize the generator and discriminator.
# generator = make_generator()
# discriminator = make_discriminator()

generator = gen_conv_model(noise_dim, sample_dim)
discriminator = dis_dense_model(sample_dim)
...
for epoch in range(300):
    ...
    print("Error ",generator_loss[-1],  discriminator_loss[-1])
    # Still needs some code to display losses from the generator and discriminator, progress bars, etc.
    generate_images(generator, args.output_dir, epoch, noise_dim)